...
class Client {
  ...
  async getFlagData() {
    const res = await axios.get(`${this.config.providerAddress}/flags?sdk_key=${this.config.sdkKey}`);
    this.setFeatureFlags(res.data);
  }

  listenForEvents() {
    const eventSource = new EventSource(`${this.config.providerAddress}/stream?sdk_key=${this.config.sdkKey}`);
